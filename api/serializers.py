...
class ApschedNodeJobsSerializer(serializers.ModelSerializer):
    ...
    def get_jobs_detail(self, obj):
        if obj.sched_type == "date":
            data = obj.to_date_json()
        elif obj.sched_type == "interval":
            data = obj.to_interval_json()
        else:
            data = obj.to_cron_json()
        return data
...
class IPVSSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = IPVS_CONFIG
        fields = (
            'id', 'vip', 'port', 'scheduler', 'sip', 'rs_count', 'persistence', 'protocol', 'line', 'desc', 'is_active',
            'ipvs_assets', 'project', 'rs_list')
