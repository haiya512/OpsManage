    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        ...
        if 'rc' in result._result and 'stderr' in result._result:
            data = "<font color='#DC143C'>{host} | FAILED | rc={rc} >> \n{stderr}</font>".format(
                host=result._host.get_name(), rc=result._result.get('rc'),
                stderr=result._result.get('stderr') + result._result.get('stdout') + result._result.get('msg'))
        else:
            data = "<font color='#DC143C'>{host} | FAILED! => {stderr}</font>".format(host=result._host.get_name(),
                                                                                      stderr=json.dumps(result._result,
                                                                                                        indent=4))
        self.save_msg(data)
    ...
    def v2_runner_on_ok(self, result, *args, **kwargs):
        ...
        if result._task.action in ('include', 'include_role', '_ansible_parsed', '_ansible_no_log'):
            ...
            if delegated_vars:
                ...
                msg = "<font color='green'>ok: [{host}] => \n{msg}</font>".format(host=result._host.get_name(),
                                                                                  msg=result._result.get('msg'))
            elif 'rc' in result._result and 'stdout' in result._result:
                msg = "<font color='green'>ok: [{host}] => \n{stdout}</font>".format(host=result._host.get_name(),
                                                                                     stdout=result._result.get(
                                                                                         'stdout'))
            else:
