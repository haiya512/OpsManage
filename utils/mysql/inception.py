...
class Inception():
    ...
    def run(self, action, auditSql):
        ...
        try:
            ...
            if check:
                ...
                return {"status": 'error', 'data': dataList}
    ...
    def getRollBackSQL(self, dbName, tableName, sequence):
        sql = '''select rollback_statement from {tableName} where opid_time="{sequence}";'''.format(tableName=tableName,
                                                                                                    sequence=sequence)
        try:
            conn = pymysql.connect(
    ...
    def getRollBackTable(self, dbName, sequence):
        sql = '''SELECT tablename from  $_$inception_backup_information$_$ where opid_time = '{sequence}';'''.format(
            sequence=sequence)
        try:
            conn = pymysql.connect(
                host=self.incept.backup_host, user=self.incept.backup_user,
