...
class CronManage(LoginRequiredMixin, CrontabManage, View):
    ...
    @method_decorator_adaptor(permission_required, "sched.can_read_cron_config", "/403/")
    ...
    @method_decorator_adaptor(permission_required, "sched.can_add_cron_config", "/403/")
    def post(self, request, *args, **kwagrs):
        if request.POST.get('type') == 'view':
            res = self.view_logs(request)
        elif request.POST.get('type') == 'disabled':
            res = self.disabled(request)
        elif request.POST.get('type') == 'enable':
            res = self.enable(request)
        else:
            res = self.create_crontab(request=request)
        if isinstance(res, str): return JsonResponse({'msg': res, "code": 500, 'data': []})
        return JsonResponse({'msg': "操作成功", "code": 200, 'data': res})

    @method_decorator_adaptor(permission_required, "sched.can_change_cron_config", "/403/")
    def put(self, request, *args, **kwagrs):
        res = self.update_crontab(request=request)
        if isinstance(res, str): return JsonResponse({'msg': res, "code": 500, 'data': []})
        return JsonResponse({'msg': "修改成功", "code": 200, 'data': res})

    @method_decorator_adaptor(permission_required, "sched.can_delete_cron_config", "/403/")
