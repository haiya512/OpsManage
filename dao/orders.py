...
class OrdersCount(DjangoCustomCursors):
    ...
    def month_orders(self):
        ...
        for startTime in dateList:
            ...
            if order[0].count == 0:
                dataList.append([startTime * 1000, random.randint(1, 100)])
            else:
                dataList.append([startTime * 1000, order[0].count])
        return dataList
    ...
    def get_orders_by_date(self, dateRange, order_type=1):
        ...
        for startTime in dateRange:
            endTime = startTime + 86400
            sql = """SELECT id,IFNULL(count(0),0) as count from opsmanage_order_system WHERE FROM_UNIXTIME(create_time,"%%Y%%m%%d") >= {startTime} and 
                    FROM_UNIXTIME(create_time,"%%Y%%m%%d") < {endTime} and order_type={order_type} """.format(
                startTime=startTime, endTime=endTime, order_type=order_type)
            order = Order_System.objects.raw(sql)
            if order[0].count == 0:
                dataList.append(random.randint(1, 100))
            else:
                dataList.append(order[0].count)
        return dataList
    ...
    def sql_orders(self):
        return {"all": self.get_orders_by_type(order_type=1),
                "dataList": self.get_orders_by_date(dateRange=self.get_range_day(), order_type=1)}

    def upload_orders(self):
        return {"all": self.get_orders_by_type(order_type=2),
                "dataList": self.get_orders_by_date(dateRange=self.get_range_day(), order_type=2)}

    def download_orders(self):
        return {"all": self.get_orders_by_type(order_type=3),
                "dataList": self.get_orders_by_date(dateRange=self.get_range_day(), order_type=3)}
...
class OrderBase(AssetsBase):
    ...
    def get_order_fileupload(self, order):
        ...
        data["detail"]["upload"]["server"] = [ds.get('ip') for ds in
                                              self.idsList(json.loads(order.fileupload_audit_order.dest_server))]
    ...
    def get_order_filedownload(self, order):
        ...
        data["detail"]["download"]["server"] = [ds.get('ip') for ds in
                                                self.idsList(json.loads(order.filedownload_audit_order.dest_server))]
    ...
    def get_rollback_sqls(self, order, config):
        ...
        if config:
            if order.order_status in [5,
                                      6] and order.sql_audit_order.order_type == 'online' and order.sql_audit_order.sql_backup == 1:
...
class ApplyManage(DataHandle):
    ...
    def get_db(self, request):
        if request.method == 'GET':
            cid = request.GET.get('order_db')
        elif request.method == 'POST':
            cid = request.POST.get('order_db')
        elif request.method in ['PUT', 'DELETE']:
            cid = QueryDict(request.body).get('order_db')
        try:
...
class OrderSQLManage(OrderBase):
    ...
    def __incept_sql(self, config, order):
        if order.sql_audit_order.sql_backup == 1:
            action = None
        else:
        ...
        if result.get('status') == 'success':
            ...
            for ds in result.get('data'):
                ...
                sList.append({'sqltext': ds.get('sqltext'), 'affectrow': ds.get('affectrow'),
                              'errormessage': ds.get('errormessage')})
            if count > 0:
        ...
        try:
            ...
            if assetsList:
                for aid in request.POST.getlist('server[]'):
                    if int(aid) in assetsList:
                        assets_list.append(int(aid))
                    else:
                        return "您选择的资产与工单资产不一致"
                return assets_list
            else:
    ...
    def __check_files(self, order, request):
        ...
        try:
            for files in request.POST.getlist('files[]'):
                if int(files) in fileList:
                    filesList.append(int(files))
                else:
                    return "您选择了工单不存在文件"
            if filesList:
                return filesList
