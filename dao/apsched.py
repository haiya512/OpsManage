...
class ApschedNodeManage(AssetsBase):
    ...
    def schedNode(self, request):
        if request.method == 'GET':
            cid = request.GET.get('sched_node')
        elif request.method == 'POST':
            cid = request.POST.get('sched_node')
        elif request.method in ['PUT', 'DELETE']:
            cid = QueryDict(request.body).get('sched_node')
        try:
    ...
    def get_node_jobs_by_token(self, token):
        ...
        for job in node.node_jobs.all():
            if job.status == "running":
                if job.sched_type == "date":
                    data = job.to_date_json()
                elif job.sched_type == "interval":
                    data = job.to_interval_json()
                else:
                    data = job.to_cron_json()
...
class ApschedNodeJobsManage(ApschedNodeManage):
    def __init__(self):
        self.jobs_sched_field = ["second", "minute", "hour", "week", "day", "day_of_week", "month", "start_date",
                                 "end_date", "run_date"]
    ...
    def rpc_update_jobs(self, jobs, uri):
        ...
        result = sched_rpc.post(
            url="http://{ip}:{port}/api/v1/{uri}".format(ip=jobs.job_node.sched_server.server_assets.ip,
