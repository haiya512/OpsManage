...
class AppsCount:
    ...
    def get_apps_release_status(self, project=None):
        dataList = []
        if project:
            sql = """select id,count(1) as count,`status` from opsmanage_log_project_config where project_id={project} and `status` IN ("success","failed") GROUP BY `status`;""".format(
                project=project)
        else:
            sql = """select id,count(1) as count,`status` from opsmanage_log_project_config where  `status` IN ("success","failed") GROUP BY `status`;"""
    ...
    def get_apps_release_type(self, project=None):
        dataList = []
        if project:
            sql = """select id,count(1) as count,type from opsmanage_log_project_config where project_id={project} and type IN ("deploy","rollback") GROUP BY type;""".format(
                project=project)
        else:
            sql = """select id,count(1) as count,type from opsmanage_log_project_config where type IN ("deploy","rollback") GROUP BY type;"""
        for ds in Log_Project_Config.objects.raw(sql):
            data = {}
            if ds.type == "deploy":
                data["label"] = u"部署"
        ...
        for startTime in dateRange:
            ...
            if project:
                sql = """SELECT id,IFNULL(count(0),0) as count from opsmanage_log_project_config WHERE  date_format(create_time,"%%Y%%m%%d") >= FROM_UNIXTIME({startTime},'%%Y%%m%%d') and 
                                        date_format(create_time,"%%Y%%m%%d") < FROM_UNIXTIME({endTime},'%%Y%%m%%d') and project_id={project} ;""".format(
                    startTime=startTime, endTime=endTime, project=project)
            else:
                sql = """SELECT id,IFNULL(count(0),0) as count from opsmanage_log_project_config WHERE date_format(create_time,"%%Y%%m%%d") >= FROM_UNIXTIME({startTime},'%%Y%%m%%d') and 
                                        date_format(create_time,"%%Y%%m%%d") < FROM_UNIXTIME({endTime},'%%Y%%m%%d');""".format(
                    startTime=startTime, endTime=endTime)
            release = Log_Project_Config.objects.raw(sql)
            if release[0].count == 0:
                dataList.append(
                    {"dtime": base.changeTimestampTodatetime(startTime, '%Y-%m-%d'), "value": random.randint(1, 100)})
            else:
                dataList.append(
                    {"dtime": base.changeTimestampTodatetime(startTime, '%Y-%m-%d'), "value": release[0].count})
        return dataList
...
class AppsManage(AssetsBase):
    ...
    def get_task(self, request):
        if request.method == 'GET':
            cid = request.GET.get('tasks_id')
        elif request.method == 'POST':
            cid = request.POST.get('tasks_id')
        elif request.method in ['PUT', 'DELETE']:
            cid = QueryDict(request.body).get('tasks_id')
        else:
            cid = request.get('tasks_id')
        try:
    ...
    def create_app_deploy_record(self, project, username, servers, version, task_id, status="未完成", type="deploy",
                                 branch=None, tag=None, package=None):
        try:
