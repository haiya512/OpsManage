...
class AssetsBase(DataHandle):
    ...
    def manufacturerList(self):
        try:
            return [ds.manufacturer for ds in Assets.objects.raw(
                """SELECT manufacturer,id from opsmanage_assets WHERE  manufacturer is not null GROUP BY manufacturer""")]
    ...
    def providerList(self):
        try:
            return [ds.provider for ds in Assets.objects.raw(
                """SELECT provider,id from opsmanage_assets WHERE  provider is not null GROUP BY provider""")]
        except Exception as ex:
    ...
    def modelList(self):
        try:
            return [ds.model for ds in Assets.objects.raw(
                """SELECT model,id from opsmanage_assets WHERE  model is not null GROUP BY model""")]
        except Exception as ex:
    ...
    def cpuList(self):
        try:
            return [ds.cpu for ds in Server_Assets.objects.raw(
                """SELECT cpu,id from opsmanage_server_assets WHERE  cpu is not null GROUP BY cpu""")]
        except Exception as ex:
    ...
    def systemList(self):
        try:
            return [ds.system for ds in Server_Assets.objects.raw(
                """SELECT system,id from opsmanage_server_assets WHERE  system is not null GROUP BY system""")]
        except Exception as ex:
    ...
    def kernelList(self):
        try:
            return [ds.kernel for ds in Server_Assets.objects.raw(
                """SELECT kernel,id from opsmanage_server_assets WHERE  kernel is not null GROUP BY kernel""")]
        except Exception as ex:
        ...
        for assets in assetsList:
            if hasattr(assets, 'server_assets'):
                try:
                    dataList.append(
                        {"id": assets.id,
                         "ip": assets.server_assets.ip, })  # "project": project, "service": service, "status": assets.status, "mark": assets.mark})
                except Exception as ex:
                ...
                try:
                    dataList.append(
                        {"id": assets.id,
                         "ip": assets.network_assets.ip, })  # "project": project, "service": service, "status": assets.status, "mark": assets.mark})
                except Exception as ex:
                    logger.warn(msg="id:{assets}, error:{ex}".format(assets=assets.id, ex=ex))
        return dataList
...
class AssetsCount(DjangoCustomCursors):
    ...
    def statusAssets(self):
        try:
            return [{"count": ds.count, "status": ds.status} for ds in
                    Assets.objects.raw("""SELECT id,count(*) as count,status from opsmanage_assets GROUP BY status;""")]
        except Exception as ex:
    ...
    def typeAssets(self):
        try:
            return [{"count": ds.count, "assets_type": ds.assets_type} for ds in Assets.objects.raw(
                """SELECT id,count(*)  as count,assets_type from opsmanage_assets GROUP BY assets_type;""")]
        except Exception as ex:
    ...
    def databasesAssets(self):
        dataList = []
        try:
            for ds in DataBase_Server_Config.objects.raw(
                    """SELECT id,count(*) as count,db_env from opsmanage_database_server_config GROUP BY db_env;"""):
                if ds.db_env == "beta":
                    dataList.append({"count": ds.count, "db_env": "测试环境"})
                else:
    ...
    def appsAssets(self):
        dataList = []
        try:
            for ds in Project_Config.objects.raw(
                    """SELECT id,count(id)  as count,project_env from opsmanage_project_config GROUP BY project_env;"""):
                if ds.project_env == "sit":
                    dataList.append({"count": ds.count, "project_env": "测试环境"})
                elif ds.project_env == "qa":
                    dataList.append({"count": ds.count, "project_env": "灰度环境"})
                else:
                    dataList.append({"count": ds.count, "project_env": "生产环境"})
            return dataList
        except Exception as ex:
...
class AssetsSource(object):
    ...
    def query_user_assets(self, request, assetsList):
        if request.user.is_superuser:
            return assetsList
        assets_list = [das.assets for das in
                       User_Server.objects.filter(user=request.user, assets_id__in=[ds.id for ds in assetsList])]
        return assets_list

    def custom(self, request):
        if request.method == 'POST':
            if request.POST.getlist('server', []):
                serverList = request.POST.getlist('server')
            elif request.POST.getlist('custom', []):
                serverList = request.POST.getlist('custom')
            else:
                serverList = request.POST.getlist('server[]')
    ...
    def group(self, request):
        assetsList = Assets.objects.select_related().filter(group=request.POST.get('group'),
...
class AssetsAnsible(DataHandle):
    ...
    def query_user_assets(self, request, assetsList):
        if request.get('is_superuser'):
            return assetsList
        assets_list = [das.assets for das in
                       User_Server.objects.filter(user=request.get('user'), assets_id__in=[ds.id for ds in assetsList])]
        return assets_list
    ...
    def group(self, request):
        assetsList = Assets.objects.select_related().filter(group=request.get('group'),
                                                            assets_type__in=["server", "vmser", "switch", "route"])
        return self.source(self.query_user_assets(request, assetsList))
