...
class DBUser(object):
    def __init__(self):
        ...
        self.user_grants = ["create", "drop", "alter", "truncate", "insert", "update", "delete", "select", "show",
                            "desc", "explain", "rename"]
    ...
    def delete_user_database(self, request):
        try:
            Database_User.objects.get(db=QueryDict(request.body).get('id'),
                                      user=QueryDict(request.body).get('uid')).delete()
        except Exception as ex:
    ...
    def get_user_db_grants(self, request):
        ...
        try:
            if userDbServer.privs:
                user_grants = userDbServer.privs.split(",")
            else:
                user_grants = []
        except Exception as ex:
...
class DBManage(AssetsBase):
    ...
    def query_sql(self, request):
        ...
        sql_parse = self.__check_sql_parse(request, allow_sql=["select", "show", "desc", "explain"],
                                           dbname=dbServer.get('db_name'))
    ...
    def parse_sql(self, request):
        ...
        try:
            ...
            conn_setting = {'host': dbServer.db_assets.server_assets.ip, 'port': dbServer.db_port,
    ...
    def recursive_node_to_dict(self, node, request):
        ...
        if json_format["last_node"] == 1:
            db_children = []
            for ds in DataBase_Server_Config.objects.filter(db_business=json_format["id"],
    ...
    def tree(self, request):
        if request.user.is_superuser:
            ...
            user_business = [ds.get("db_business") for ds in
                             DataBase_Server_Config.objects.values('db_business').annotate(
                                 dcount=Count('db_business',
                                              filter=Q(
                                                  id__in=[
                                                      ds.db
                                                      for ds
                                                      in
                                                      Database_User.objects.filter(
