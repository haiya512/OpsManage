...
class Config(LoginRequiredMixin, AppsManage, View):
    ...
    @method_decorator_adaptor(permission_required, "cicd.read_project_config", "/403/")
    def config(self, request, *args, **kwagrs):
        return render(request, 'cicd/cicd_config.html',
                      {"user": request.user, "assets": self.base(), "project_dir": settings.WORKSPACES})
    ...
    @method_decorator_adaptor(permission_required, "cicd.change_project_config", "/403/")
    ...
    def edit(self, request, *args, **kwagrs):
        return render(request, 'cicd/cicd_edit.html',
                      {"user": request.user, "assets": self.base(), "project_dir": settings.WORKSPACES})

    @method_decorator_adaptor(permission_required, "cicd.add_project_config", "/403/")
...
class Manage(LoginRequiredMixin, AppsManage, AssetsSource, View):
        ...
        if project:
            ...
            if logPaths and sList and logPaths in logPathsList:
                if request.POST.get('keywords'):
                    module_args = """grep "{keywords}" {log_path}| tail -n 1000 """.format(
                        keywords=request.POST.get('keywords'), log_path=logPaths)
                else:
                    module_args = """tail -n 1000 {log_path}""".format(log_path=logPaths)
    ...
    def download_package(self, request):
        ...
        if project and task:
            ...
            response['Content-Disposition'] = 'attachment; filename="{file_name}'.format(
                file_name=os.path.basename(task.package))
            return response
        else:
    ...
    @method_decorator_adaptor(permission_required, "cicd.delete_project_config", "/403/")
    ...
    def histroy(self, request):
        version, project = self.apps_type(request)
        result = version.show(path=project.project_repo_dir, branch=request.GET.get('project_branch'),
                              cid=request.POST.get('project_version', None))
        return JsonResponse(
            {'msg': "操作成功", "code": 200, 'data': "<pre> <xmp>" + result[1].replace('<br>', '\n') + "</xmp></pre>"})
