...
class DeployRunner(AssetsSource):
    ...
    def __init__(self, apps_id=None, task=None, ws=None):
        ...
        if task:
            ...
            self.release_version = '{code_base}{version}/'.format(
                code_base=WORKSPACES.rstrip(
                    '/') + '/deploy/' + self.apps.project_name + '/' + self.apps.project_env + '/',
                version=version
            )
    ...
    def handle_result(self, progress, result):
        if isinstance(result, list):
            for ds in result:
                ...
                self.ws.save_deploy_logs(key=progress, msg=msg, title=progress, task_id=self.task.task_id,
            ...
            if result.get("status") == "failed":
                ...
                self.update_task_status(status='已失败')
                self.ws.send(
                    json.dumps({"progress": progress, "msg": result.get("msg"), "status": result.get("status")}))
                self.ws.disconnect(1000)

            self.ws.save_deploy_logs(key=progress, msg=result.get("msg"), title=progress, task_id=self.task.task_id,
    ...
    def post_deploy(self):
        ...
        self.handle_result("post_deploy", base.cmds(
            cmds='cd {project_repo_dir} && bash {tmpFile}'.format(tmpFile=tmpFile,
                                                                  project_repo_dir=self.release_version)))
    ...
    def pre_release(self):
        ...
        if self.apps.project_pre_remote_command:
            ...
            ansRbt.run_model(host_list=self.judge_servers(hostList), module_name='shell',
                             module_args=self.apps.project_pre_remote_command)
            # 精简返回的结果
            dataList = ansRbt.handle_model_data(ansRbt.get_model_result(), 'shell',
                                                module_args=self.apps.project_remote_command)
            self.handle_result("pre_release", dataList)

    def release(self, package=None):
        if package:
            args = "src={srcDir} dest={desDir} mode=755 owner={owner}  group={owner}".format(
                srcDir=package,
                desDir=self.apps.project_target_root,
                owner=self.apps.project_user)
        else:
            args = "src={srcDir} dest={desDir} mode=755 owner={owner}  group={owner}".format(
                srcDir=self.apps.project_dir + self.version_package, desDir=self.apps.project_target_root,
                owner=self.apps.project_user)
        hostList, resource = self.idSourceList(ids=self.task.servers)
        ansRbt = ANSRunner(resource)
    ...
    def post_release(self):
        ...
        if self.apps.project_remote_command:
            ...
            ansRbt = ANSRunner(resource)
            #             print(list(set(hostList).difference(set(self.flist))))
            ansRbt.run_model(host_list=self.judge_servers(hostList), module_name='raw',
                             module_args=self.apps.project_remote_command)
            # 精简返回的结果
            dataList = ansRbt.handle_model_data(ansRbt.get_model_result(), 'raw',
                                                module_args=self.apps.project_remote_command)
