...
class IPVS_CONFIG(models.Model):
    ...
    def add_vip(self):
        if self.persistence:
            return "ipvsadm -A {protocol} {vip}:{port} -s {scheduler} -p {persistence}".format(protocol=self.protocol,
                                                                                               vip=self.vip,
                                                                                               port=self.port,
                                                                                               scheduler=self.scheduler,
                                                                                               persistence=self.persistence)
        return "ipvsadm -A {protocol} {vip}:{port} -s {scheduler}".format(protocol=self.protocol, vip=self.vip,
                                                                          port=self.port, scheduler=self.scheduler)

    def modf_vip(self):
        if self.persistence:
            return "ipvsadm -E {protocol} {vip}:{port} -s {scheduler} -p {persistence}".format(protocol=self.protocol,
                                                                                               vip=self.vip,
                                                                                               port=self.port,
                                                                                               scheduler=self.scheduler,
                                                                                               persistence=self.persistence)
        return "ipvsadm -E {protocol} {vip}:{port} -s {scheduler}".format(protocol=self.protocol, vip=self.vip,
                                                                          port=self.port, scheduler=self.scheduler)
    ...
    def stats_vip(self):
        return "ipvsadm -ln {protocol} {vip}:{port} --stats".format(protocol=self.protocol, vip=self.vip,
                                                                    port=self.port)
...
class IPVS_RS_CONFIG(models.Model):
    ...
    rs_assets = models.ForeignKey('asset.Assets', related_name='ipvs_realserver_total', on_delete=models.CASCADE,
                                  verbose_name='assets_id')
    ...
    def add_realsever(self):
        return "ipvsadm -a {protocol} {vip}:{port} -r {ipvs_fw_ip}:{port} {forword} -w {weight}".format(
            vip=self.ipvs_vip.vip, port=self.ipvs_vip.port,
            protocol=self.ipvs_vip.protocol, ipvs_fw_ip=self.ipvs_fw_ip,
            forword=self.forword, weight=self.weight)

    def modf_realsever(self):
        return "ipvsadm -e {protocol} {vip}:{port} -r {ipvs_fw_ip}:{port} {forword} -w {weight}".format(
            vip=self.ipvs_vip.vip, protocol=self.ipvs_vip.protocol,
            port=self.ipvs_vip.port, ipvs_fw_ip=self.ipvs_fw_ip,
            forword=self.forword, weight=self.weight)

    def del_realsever(self):
        return "ipvsadm -d {protocol} {vip}:{port} -r {ipvs_fw_ip}:{port}".format(vip=self.ipvs_vip.vip,
                                                                                  protocol=self.ipvs_vip.protocol,
                                                                                  port=self.ipvs_vip.port,
                                                                                  ipvs_fw_ip=self.ipvs_fw_ip)


class IPVS_NS_CONFIG(models.Model):
    ipvs_vip = models.ForeignKey('IPVS_CONFIG', related_name='ipvs_ns_total', on_delete=models.CASCADE,
                                 verbose_name='vip_id')
...
class IPVS_OPS_LOG(models.Model):
    ipvs_vip = models.ForeignKey('IPVS_CONFIG', related_name='ipvs_ops_total', on_delete=models.CASCADE,
                                 verbose_name='vip_id')
