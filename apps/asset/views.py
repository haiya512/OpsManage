...
class AssetsList(LoginRequiredMixin, AssetsBase, View):
    ...
    def get(self, request, *args, **kwagrs):
        return render(request, 'assets/assets_list.html',
                      {"user": request.user, "assets": self.base(), "assetsList": self.assetsList()})
...
class AssetsServer(LoginRequiredMixin, AssetsBase, View):
    ...
    def post(self, request, *args, **kwagrs):
        return JsonResponse({'msg': "主机查询成功", "code": 200,
                             'data': self.allowcator(request.POST.get('query'), request.POST.get('id'), request)})
...
class AssetsBatch(LoginRequiredMixin, AssetsSource, View):
    ...
    def post(self, request, *args, **kwagrs):
        ...
        if sList:
            ...
            return JsonResponse({'msg': fList, "code": 500, 'data': {"success": sList, "failed": fList}})
