...
def AnsibleScripts(**kw):
    ...
    try:
        if 'scripts_id' in kw.keys():
            ...
            if 'logs' in kw.keys():
                logId = DeployRecord.Model.insert(user='celery', ans_model='script', ans_server=','.join(sList),
                                                  ans_args=filePath)
            ANS = ANSRunner(resource, background=logId)
            ANS.run_model(host_list=sList, module_name='script',
                          module_args="{filePath} {args}".format(filePath=filePath, args=script.script_args))
