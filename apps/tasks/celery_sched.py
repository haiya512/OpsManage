        ...
        if assets.assets_type in ['vmser', 'server']:
            try:
                ...
                if server_assets.keyfile != 1:
                    resource.append(
                        {"hostname": server_assets.ip, "ip": server_assets.ip, "port": int(server_assets.port),
                         "username": server_assets.username, "password": server_assets.passwd})
                elif server_assets.keyfile_path:
                    resource.append({"hostname": server_assets.ip, "ip": server_assets.ip, "port": server_assets.port,
                                     "username": server_assets.username, "password": server_assets.passwd,
                                     "private_key": server_assets.keyfile_path})
