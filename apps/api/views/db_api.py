...
def db_org(request, id, format=None):
    ...
    if request.method == 'POST':
        ...
        if dbServer.db_mode == 'pxc':
            data = ARCH_INFO.pxc()
        elif dbServer.db_mode == 'slave':
            data = ARCH_INFO.master_slave()
        else:
...
def db_user_server_dblist(request, uid, sid, format=None):
    ...
    if request.method == 'GET':
        ...
        all_user_db_list = [ds.db for ds in
                            Database_User.objects.filter(user=uid, db__in=[ds.id for ds in db_server.databases.all()])]
...
def db_user_db_table_list(request, uid, did, format=None):
    ...
    if request.method == 'GET':
        ...
        if not isinstance(result, str):
            ...
            try:
                if user_db.tables:
                    grant_tables = user_db.tables.split(",")
                else:
                    grant_tables = []
            except:
                grant_tables = []
